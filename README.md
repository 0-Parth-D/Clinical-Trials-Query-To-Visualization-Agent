...
| `drug_name` | string | no | Intervention/drug hint; merged into CT.gov query and can drive comparisons. |
...
| `extra_filters` | object | no | Forward-compatible bag; e.g. `{"comparison_drug": "Nivolumab"}` for two-drug comparisons. |

Structured fields **override** the same keys from LLM/heuristic intent when supplied.
...
- If `OPENAI_API_KEY` is set, the service asks the model for a **JSON intent only** (filters + `viz_goal`). **Counts are never taken from the model**; aggregation is deterministic over API results.
- If the key is missing or the call fails, a **heuristic** classifier uses keywords in `query` (e.g. “network”, “trend”, “vs”, “recruiting”) plus structured fields.
...
- **Grouped drug comparison** — requires `drug_name` and `comparison_drug` (or `extra_filters.comparison_drug`). The CT.gov query uses an `OR` on intervention names when both are set.
- **403/CAPTCHA** — some networks block automated access to `clinicaltrials.gov`. Use a normal consumer network or VPN; User-Agent is set on our client.
...
- [app/intent/](app/intent/) — LLM + heuristic intent
